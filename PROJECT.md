...
**Version:** v11.02 / SW v175 — pushed June 24, 2026. **GPI/Index de-branding pass.** Fixed a benchmark-table data-shift bug, then dropped "GPI"/"Index" language from player-facing UI in favour of plain "Strokes Gained" terminology (the industry-standard metric, vs. the home-grown GPI hybrid). `settings.html` (v1.22): Tracking Your Performance section reduced to 3 areas (BS/SG/Putting), sloppy footnote removed, toggle relabelled "Track Your Golf Performance" (internal `id`/`onchange` of `tog-statGPI`/`statGPI` left untouched so existing localStorage data isn't orphaned). `apps-script.gs` email report: subject simplified to flat "Golf Report"; masthead changed from "Golf Performance Index Report" + redundant "Last N rounds" byline to "Recent Performance (Mon Day, Year)"; Cost Breakdown table's last row relabelled "Strokes Gained" (was briefly "SG Rating" — caught as ambiguous against the "Short Game" row above it before shipping). Both commits (`70afd6d`, `3e630ec`) redeployed live via Apps Script editor and confirmed. Reviewed `GPI_RULES.md` for staleness; Paul's call was to keep the file on disk (not delete) but record in `CLAUDE.md` that `GPI_RULES.md` and `GPI-MetricsGuide.md` are discontinued as authoritative references as of 2026-06-24 — `apps-script.gs → buildDiagnostics_()` is now the sole source of truth for the cost model. **That `CLAUDE.md` edit is made locally but not yet committed** — Paul said "commit later."
**Known Issues:** None open. Decided-not-built: per-category stat-tracking toggles (BS/SG/Putts/etc. fine-tuning or full stat-tracking opt-out) — Paul concluded this would be guesswork without distance/dispersion data and players can self-determine which toggles to set; a short in-app write-up explaining toggle guidance was floated as a possible future idea, not committed.
...
- **Multi-course strategy** — manual entry only. No third-party API. Course data entered by the user from physical scorecards and stored in localStorage. courses.json seeds 12 Kamloops-area courses at install. User-added courses live in localStorage only (per-device, not shared). Export/Import to JSON is the backup against cache wipes.
- **GolfCourseAPI.com — dropped.** Incomplete Canadian coverage, dirty course names, subscription cost. Not worth the dependency. Decided 2026-05-10.
- **GPS proximity detection** — retained for courses in courses.json that already have coordinates (12 Kamloops courses). Not required for manually added away courses. GPS coordinates are an optional field in the Courses panel — fill in if known, leave blank if not.
- **Courses panel name — "Add Course".** Manual course entry form + saved course list. No search, no API calls.
- **HI suppressed until 20 rounds** — display "—" until 20 rounds are entered or imported. No provisional number shown before that threshold.
...
- **Multi-course support** — localStorage for 12–20 courses (~60KB total, well within limits). Course data schema: hole-by-hole par/yardage/stroke index per tee set, CR/SR, optional GPS coords. Include Export/Import to JSON file as backup against accidental Safari cache wipe.
...
## Phase 2 — Year End Report

Annual season summary generated by Apps Script + Gmail, same engine as the interval reports.

**Planned content:**
- Month-by-month bar/line charts for score, GPI, HI trend
- Individual charts per metric — FIR, GIR, UD, X-UD, Penalties, Putts
- Season averages vs. prior year (if data exists)
- GPI band breakdown — how many rounds in each band (Good / Average / Below Average / Poor)
- Triggered once per year (e.g. end of October or manually via Apps Script)

**Notes:**
- Same Apps Script sendReport() engine, different trigger and template
- Prior year comparison requires data retention — Dave's habit of deleting annually would lose this; worth flagging in onboarding
- Charts rendered as HTML in the email body (no image attachments needed)

## Phase 2 — Club Distance Card

Cached reference tool accessible from the footer nav. No network dependency.

**Data captured per club:**
- Club name (e.g. 7i, 5i, PW, Hybrid, Driver)
- Flight distance (yards)
- Rollout distance (yards)
- Total carry (auto-calculated)

**Wind adjustment table** (cached reference, not calculated dynamically):
- Into wind: drop N clubs per wind speed bracket (e.g. 20km/h = −3 clubs)
- Downwind: add N clubs per bracket
- Crosswind: note for alignment only

**Notes:**
- Manually entered once from Trackman or lesson data — stored in localStorage, per-player
- Editable via Settings panel
- Displayed as a clean reference card, readable at a glance mid-round
- Trackman-measured distances are preferred; self-estimated as fallback
- Wind brackets TBD — keep simple for recreational golfers
...
- **GolfCourseAPI.com dropped** (2026-05-10) — incomplete Canadian coverage, dirty data, subscription cost. All course data entered manually from physical scorecards.
- **courses.json** seeds 12 Kamloops-area courses at install. User-added courses live in localStorage only — per-device, not shared between users.
- **Course data accuracy** — manually entered data is as accurate as the scorecard it came from. No external data quality risk.
- **Export/Import to JSON** — backup path against Safari cache wipes. On roadmap.
- **Kamloops GC tee data** — male tee count needs verification against a physical scorecard before go-live.
- **All seeded courses** should be verified against physical scorecards before publishing.
