...
- Stats final columns: `Round ID | Date | Hole | FIR | GIR | PEN | UD | X-UD | PUTTS | Score`
...
- FIR / GIR / PEN / UD / X-UD  stay as toggles. Off = null (not tracked), on = 1.
...
- **Stats values** — 1/null for flags (FIR, GIR, PEN, UD, X-UD). Actual number for PUTTS. Score = strokes − putts computed server-side.
...
1. **Stats summary on the Posted! screen** (Paul's last idea, deferred). Fixed-height summary inside `.success-body` (already `flex: 0 0 300px`, won't shift layout). Show front/back/total, FIR/GIR/UD hit counts, average Score. Clear on New Round. ~30 min.
...
4. **Hide the footer stat controls on non-hole screens.** The setup/home screen still shows FIR/GIR/PEN/UD/X-UD toggles plus the `PUTTS` rocker. They serve no purpose outside the hole screen and are inconsistent with the hole-screen counter. Cleanest fix: hide the whole `.footer-grid` on setup, midround, card, and success — they're not interactive anywhere but hole. Small CSS-only change. This has been resolved.
